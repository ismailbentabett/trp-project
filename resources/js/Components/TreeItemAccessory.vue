<template>
    <li>
        ...
        >
            <TreeItemAccessory v-for="nestedItem in getNestedItems(item)" :key="nestedItem.id" :item="nestedItem" />
        </ul>
    </li>
</template>
...
function submitForm(form) {
    ...
    globalFilterStore.updateAccessoryTreeItem(form)

    // Step 2: Get the entire flattened tree data from the store
    const flattenedTreeData = globalFilterStore.getAccessoryTreeData
    ...
    globalFilterStore.updateAccessoryTreeData(flattenedTreeData)
}
